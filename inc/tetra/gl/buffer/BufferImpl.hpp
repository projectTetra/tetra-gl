...
template <class VertexType>
Buffer<VertexType>& Buffer<VertexType>::operator=( Buffer<VertexType>&& b )
{
...
template <class VertexType>
void Buffer<VertexType>::SetData( const std::vector<VertexType>& data, GLenum bufferUsage )
{
    ...
    // if there is an element buffer, then we let the element buffer determine
...
template <class VertexType>
void
Buffer<VertexType>::SetElementData( const std::vector<unsigned short>& data,
                                    GLenum bufferUsage )
{
    if ( !this->elementBufferHandle )
    {
...
Buffer<VertexType>&
Buffer<VertexType>::EnableVertexAttrib( GLuint index,
                            std::array<float, length> VertexType::*Attrib )
{
    // Get the byte offset of the attrib from beginning of the vertex struct
...
void Buffer<VertexType>::Draw( GLenum topology ) const
{
    ...
    if ( this->elementBufferHandle )
    {
        glDrawElements( topology, this->vertexCount, GL_UNSIGNED_SHORT, 0 );
